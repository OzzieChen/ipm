...
function ovrfw(str) {
	if (str == undefined || str == "" || str == null) {} else if (str.length > 10) {
...
// 货币分析成浮点数 alert(parseCurrency("￥1,900,000.12"));
...
function brgba(hex, opacity) {
	...
	var h = hex.charAt(0) == "#" ? hex.substring(1) : hex, r = parseInt(h.substring(0, 2), 16), g = parseInt(h.substring(2, 4), 16), b = parseInt(h.substring(4, 6), 16), a = opacity;
	return "rgba(" + r + "," + g + "," + b + "," + a + ")";
}
...
function GooFlow(bgDiv, property) {
	...
	this.$workArea = $("<div class='GooFlow_work_inner' style='width:" + width * 2 + "px;height:" + height * 2 + "px'></div>").attr({
		"unselectable" : "on",
	...
	if (this.$editable) {
		...
		this.$workArea.mouseup({
			...
			if (tmp)
				e.data.inthis.$draw.removeChild(tmp);
		});
...
GooFlow.prototype = {
	...
	addNode : function(id, json) {
		...
		if (json.type != "start" && json.type != "end" && json.type != "ln") {
			json.width = json.width ? json.width : 60;
		...
		if (type == "node") {
			...
			if (GooFlow.prototype.useSVG != "") {
				...
				if (mark)
					...
					this.$nodeDom[id].strokeColor = "#5068AE"
			}
		}
		if (this.$undoStatck) {
